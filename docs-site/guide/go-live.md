...
Обновление продукта — одной командой `./scripts/update.sh`, запущенной из папки бота: скрипт сначала снимает бэкап БД, потом тянет обновления, пересобирает и мигрирует; если новая версия не поднялась — печатает готовые команды отката. Данные не теряются. Детали — [/guide/update](/guide/update).
